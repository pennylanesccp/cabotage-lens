...
For every voyage, the enricher evaluates every ordered origin-destination port pair, including direct voyages and complete multi-stop corridors. A voyage contributes once to a given ordered pair; when repeated port calls create more than one eligible slice, the same current rule is applied within that voyage (direct first, otherwise shortest distance). Derived voyage fuel is summed over the observed sublegs using the reconstructed cargo aboard, subleg distance, and resolved voyage intensity. Corridors are never assembled from legs belonging to different voyages.

For each ordered port pair, all resolved same-OD voyage observations across every corridor contribute to two separate indicators. The pair intensity is a transport-work-weighted arithmetic mean, where each voyage weight is the sum of `cargo_onboard_t * subleg_distance_nm`. The representative scenario distance is a mean of complete observed voyage distances weighted by the voyage's distance-weighted mean cargo aboard, `transport_work_tnm / voyage_distance_nm`. This retains the cargo reconstructed on every subleg without allowing route distance to enter the distance weight a second time. Zero-work observations do not influence the pair intensity when positive work exists; when every observation has zero work, the documented unweighted contingency is used. No individual corridor is selected as the scenario path or distance.

The explicit `pair_intensity_*` and `scenario_distance_*` fields are canonical for the scenario. They record the method, the weighting basis, the supporting observations, and the provenance needed to reproduce the applied values.
...
The enricher preserves the existing `matrix` block and appends the same-OD pair estimator, evaluated corridor alternatives, subleg calculations, and intensity provenance under the directional section. The metadata declares `maritime_intensity_schema_version=7`; a complete tracked v7 artifact takes precedence over an older or incomplete remote observed-corridor artifact. When the ANTAQ tabular CSVs or MRV lookup JSON are missing locally, the pipeline can resolve them from the configured Supabase Storage data bucket. By default it prunes the distance matrix to observed usable combinations; pass `--keep-unmatched-pairs` to retain observed pairs with unresolved intensity, or `--keep-all-matrix-pairs` to retain the full original distance matrix.
