...
Enrich the repository sea matrix with directional MRV fuel-per-transport-work intensities derived from complete observed ANTAQ voyage corridors:
...
For every voyage, the enricher evaluates every ordered origin-destination port pair, including direct voyages and complete multi-stop corridors. A voyage contributes once to a given ordered pair; when repeated port calls create more than one eligible slice, the same current rule is applied within that voyage (direct first, otherwise shortest distance). Derived voyage fuel is summed over the observed sublegs using the reconstructed cargo aboard, subleg distance, and resolved voyage intensity. Corridors are never assembled from legs belonging to different voyages. Across voyages, the current selection rule remains explicit: prefer a usable observed direct corridor; otherwise select the shortest complete observed corridor for path and distance.

The scenario intensity is intentionally separated from that geometry selection. For each ordered port pair, all resolved same-OD voyage observations across every corridor contribute to a transport-work-weighted median. Each voyage weight is the sum of `cargo_onboard_t * subleg_distance_nm` between the pair. The deterministic estimate is the first sorted intensity whose cumulative positive transport-work weight reaches 50%. Zero-work observations do not influence the weighted median when positive work exists; if every resolved observation has zero work, an explicitly labeled unweighted median is used. The selected corridor supplies only its observed port sequence and distance, and the pair intensity is applied to every selected subleg distance. This rule is not calibrated to reproduce a historical route value.

The explicit `pair_intensity_*` fields are canonical for the scenario. Legacy weighted-mean and subleg intensity fields retain their selected-corridor meaning; applied values and provenance use separate scenario fields. The all-zero-work case has its own unweighted-median method and source rather than being labeled as transport-work weighted.
...
The voyage-stop input is the normalized containerized-cargo call sequence produced by the ANTAQ pipeline; it should not be read as an inventory of physical calls with no observed container movement. When a resolved voyage or subleg has zero derived transport work, the diagnostic fuel attributed to that observed activity remains zero. Corridor-local diagnostics retain their explicitly labeled zero-work behavior, while the pair estimator excludes zero weights whenever positive transport work exists and otherwise uses the labeled unweighted-median fallback described above.

Intensity resolution is recorded per voyage. The hierarchy is an exact latest positive EU MRV match by IMO, then an available vessel-class statistic, then a ship-type statistic (the container-ship type is the documented default for the containerized ANTAQ scope). Class fallbacks use the tracked `trimmed_mean_1pct` and fall back to the class median only when that field is unavailable. Ship-type fallbacks symmetrically remove the lowest and highest 1% of latest positive IMO values (integer floor per tail) before taking the mean; samples too small to remove at least one observation per tail use the median. Exact IMO values are never trimmed. Ship-type fallback provenance records the statistic, rule, raw/retained sample sizes, excluded count, bounds, and untrimmed summaries; class provenance records the statistic and available sample metadata from its tracked artifact. Aggregate indicators keep exact IMO matches separate from class/type fallbacks.

The enricher preserves the existing `matrix` block and appends the same-OD pair estimator, selected-corridor statistics, evaluated corridor alternatives, subleg calculations, and intensity provenance under the directional section. The metadata declares `maritime_intensity_schema_version=3`; a complete tracked v3 artifact takes precedence over an older or incomplete remote observed-corridor artifact, including an older v3 generation timestamp. When the ANTAQ tabular CSVs or MRV lookup JSON are missing locally, the pipeline can resolve them from the configured Supabase Storage data bucket. By default it prunes the distance matrix to observed usable combinations; pass `--keep-unmatched-pairs` to retain observed pairs with unresolved intensity, or `--keep-all-matrix-pairs` to retain the full original distance matrix.
