...
from app.heatmap.service import HeatmapDataError, list_origin_options, load_latest_dataset
...
class HeatmapServiceTests(unittest.TestCase):
    ...
    def test_load_latest_dataset_raises_when_rows_cannot_be_mapped(self) -> None:
        run_record = SimpleNamespace(
            run_id="run-456",
            origin_name="Pelotas, RS",
            cargo_t=30.0,
        ...
        with patch("app.heatmap.service._require_postgres"), patch(
            ...
            with self.assertRaises(HeatmapDataError):
                load_latest_dataset(HeatmapScenario(origin_name="Pelotas, RS", cargo_t=30.0))
