...
def render_advanced(class_options: Iterable[str], port_ops_scenarios: Iterable[str]) -> None:
    ...
    st.caption("Sea leg visualization uses a single 60-degree circular arc between the selected ports.")
    st.slider("Sea arc points", min_value=60, max_value=400, step=10, key="map_sea_n_points", disabled=not bool(st.session_state.map_show_sea))
