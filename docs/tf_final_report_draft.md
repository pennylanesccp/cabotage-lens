...
- expor a proveniencia das distancias, da selecao de portos e dos componentes de operacoes portuarias/*hoteling*, especialmente quando ha fallback, caso same-port, porto alternativo, referencia exata ausente ou fonte portuaria de menor confianca;
...
O tratamento metodologico desses componentes tambem exige distinguir qualidade de fonte. Quando ha registros portuarios especificos, eles constituem a base preferencial. Quando dados diretos faltam, a lacuna nao e interpretada como emissao nula: o modelo pode recorrer a media ponderada de portos observados, a um padrao documentado existente ou, se nenhum valor defensavel existir, marcar o componente como indisponivel. Assim, a literatura justifica a relevancia de operacoes em porto e seus limites, mas nao autoriza a inserir novos fatores numericos nem a tratar estimativas de fallback como equivalentes a observacoes portuarias.
