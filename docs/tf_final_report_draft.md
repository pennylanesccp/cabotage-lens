...
A interface Streamlit transforma essa memória técnica em leitura de cenário. Os cartões mostram distâncias, proxy de custo e emissões TTW CO2e. As tabelas de detalhes mostram pernas, portos, premissas, fonte de diesel, preço marítimo, fronteira de custo, fronteira de emissões, proveniência marítima, inclusão/exclusão de componentes e avisos. Os registros CSV/JSON de validação ou exportação preservam entradas, saídas, status, bloqueios, sensibilidades e classificações quando o fluxo os produz.
