...
- **Port Operations (Origin):** Port-side handling and berth-side hoteling/auxiliary fuel or emissions associated with loading, with source provenance retained for observed, estimated, documented-default, or unavailable values.
- **Maritime Sailing (Cabotage):** Coastal shipping from the port of origin to the port of destination.
- **Port Operations (Destination):** Port-side handling and berth-side hoteling/auxiliary fuel or emissions associated with unloading, with the same source-provenance hierarchy.
...
- **Port Operations:** Port handling, hoteling, and vessel auxiliary energy represented by the selected backend model. Port-specific observations are used when available; missing values are not interpreted as zero and are resolved through the documented fallback hierarchy where defensible.
...
- **Port Emissions Coverage:** Port-operation and hoteling values have lower confidence when they rely on weighted peer averages or documented defaults rather than observed port-specific data. Components marked `unavailable` are explicitly not included without defensible data, and the result should be interpreted as incomplete for that component.
