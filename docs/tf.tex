...
Considere uma viagem \(v\) e o conjunto \(\mathcal{S}_{v,o,d}\) de subtrechos contíguos entre a origem \(o\) e o destino \(d\). Para cada subtrecho \(s\), sejam \(d_s\) a distância marítima, \(m_s\) a carga reconstruída a bordo e \(I_v\) a intensidade resolvida para o navio, em g/(t$\cdot$nm). O trabalho de transporte da viagem no corredor é:
\begin{equation}
W_{v,o,d} = \sum_{s \in \mathcal{S}_{v,o,d}} m_s \cdot d_s.
\label{eq:segment-weight}
\end{equation}

O consumo observado atribuído à carga conteinerizada nessa viagem é calculado pela soma dos subtrechos:
\begin{equation}
F_{v,o,d} = \frac{1}{1000}
\sum_{s \in \mathcal{S}_{v,o,d}} I_v \cdot m_s \cdot d_s.
\label{eq:route-intensity}
\end{equation}
Viagens com a mesma sequência portuária formam uma alternativa de corredor. Sua intensidade é a razão entre o combustível somado e o trabalho de transporte somado. Quando todo o trabalho observado de um subtrecho ou corredor é zero, preserva-se a média aritmética das intensidades resolvidas, explicitamente marcada no artefato, enquanto o combustível observado permanece zero. Essa regra permite avaliar uma carga de cenário sem converter uma divisão \(0/0\) em ausência de intensidade.

Antes do cálculo, a intensidade é procurada pelo IMO no EU MRV. Sem correspondência positiva, utiliza-se primeiro a média aparada em 1\% por cauda já registrada para a classe, com mediana apenas quando esse campo não existe. Para o tipo de navio, ordenam-se os últimos valores positivos por IMO, exclui-se de cada cauda \(\lfloor 0{,}01n \rfloor\) observações e calcula-se a média da amostra retida; quando a amostra é pequena demais para remover ao menos uma observação por cauda, usa-se a mediana. Para o recorte conteinerizado sem metadado de tipo, \emph{container ship} é o fallback documentado. Valores associados diretamente ao IMO nunca são aparados. A fonte, o estimador, a regra, os tamanhos bruto e retido, a contagem excluída e os limites retidos permanecem registrados por viagem. O modelo avalia todas as sequências observadas dentro de uma mesma viagem, sem montar um caminho sintético com pernas de viagens diferentes, e seleciona primeiro um corredor direto utilizável ou, na sua ausência, o corredor completo de menor distância.
...
O filtro permanece restrito a corredores presentes na sequência normalizada de uma mesma viagem. Nenhum subtrecho foi descartado por nome de terminal, distância ausente ou intensidade não resolvida no artefato atual. Quatro nomes de terminais foram associados a seus portos canônicos; chamadas consecutivas no mesmo complexo foram colapsadas somente depois de somar seus movimentos de carga. Para pares canônicos distintos sem distância positiva na matriz, o fallback haversine é registrado como aproximação. As sequências normalizadas derivam das chamadas com movimentação conteinerizada disponíveis no pipeline e, portanto, não devem ser interpretadas como inventário de toda atracação física sem carga observada.

\section{Lógica de avaliação marítima no motor do projeto}

Com a nova \texttt{sea\_matrix} enriquecida, a avaliação unitária carrega o corredor completo previamente selecionado no processamento offline. Para uma carga de cenário \(M\), o combustível marítimo de navegação é calculado pela soma de seus subtrechos:
\begin{equation}
F_{\text{mar}} = \frac{1}{1000}
\sum_{s \in \mathcal{S}^{*}_{o,d}} \bar{I}_s \cdot M \cdot d_s,
\label{eq:sea-fuel-route}
\end{equation}
em que \(\mathcal{S}^{*}_{o,d}\) é a cadeia selecionada, \(\bar{I}_s\) é a intensidade agregada do subtrecho e \(d_s\) é sua distância em milhas náuticas. A carga ANTAQ permanece como dado diagnóstico da construção da intensidade; ela não substitui a carga informada no cenário.

O modelo atual é hierárquico:

\begin{enumerate}
  \item resolve a intensidade de cada viagem por IMO, classe ou tipo, nessa ordem;
  \item agrega todos os subtrechos de cada viagem e todas as viagens do mesmo corredor;
  \item seleciona entre corredores completos observados sem costurar viagens distintas;
  \item atribui combustível à carga do cenário pela soma dos subtrechos selecionados;
