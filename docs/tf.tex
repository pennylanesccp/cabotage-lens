...
Antes do cálculo, a intensidade é procurada pelo IMO no EU MRV. Sem correspondência positiva, utiliza-se a média disponível da classe e depois a média do tipo de navio; para o recorte conteinerizado sem metadado de tipo, \emph{container ship} é o fallback documentado. A fonte efetivamente usada permanece registrada por viagem. O modelo avalia todas as sequências observadas dentro de uma mesma viagem, sem montar um caminho sintético com pernas de viagens diferentes, e seleciona primeiro um corredor direto utilizável ou, na sua ausência, o corredor completo de menor distância.
...
Em segundo lugar, a cobertura exata por IMO no MRV é parcial. Das \num{1324} viagens, \num{788} usam intensidade específica por IMO e \num{536} dependem, no artefato atual, da média do tipo \emph{container ship}; o fallback por classe também é suportado, mas não foi acionado pelos metadados disponíveis nesse recorte. A taxa de resolução numérica é, portanto, completa, mas a qualidade da evidência não é homogênea. Resultados com fallback devem ser interpretados com incerteza maior do que aqueles associados diretamente ao IMO.
