...
Esses números mostram que a cobertura exata por IMO ainda é parcial, mas já suficientemente extensa para sustentar estimadores direcionais por par, desde que as viagens sem correspondência sejam identificadas separadamente como fallback de classe ou tipo.
...
Com a nova \texttt{sea\_matrix} enriquecida, a avaliação unitária carrega o corredor completo previamente selecionado no processamento offline. Para uma carga de cenário \(M\), o combustível marítimo de navegação é calculado pela soma das distâncias de seus subtrechos e pela intensidade representativa do par:
\begin{equation}
...
Segundo, o modelo marítimo passou a incorporar indicadores anuais reportados por navio a partir do EU MRV, usando um KPI de combustível por \emph{transport work}. Quando o IMO não possui correspondência positiva, essa intensidade é identificada como fallback estatístico de classe ou tipo. A escolha do KPI evita confundir consumo absoluto anual do navio com intensidade normalizada pelo serviço de transporte.

Terceiro, a \texttt{sea\_matrix} agora funciona não apenas como matriz de distâncias, mas também como matriz de intensidade energética direcional derivada das viagens ANTAQ e das intensidades MRV ou de seus fallbacks identificados. Essa mudança é conceitualmente relevante: o motor deixa de operar exclusivamente com um único parâmetro de frota e passa a dispor de uma mediana por par origem--destino ponderada pelo trabalho de transporte, mantendo os corredores observados como alternativas auditáveis de caminho e distância.
...
Se fosse necessário resumir a contribuição metodológica do presente estágio do projeto em uma frase, ela seria a seguinte: o CabotageLens passou de um modelo marítimo parametrizado por uma classe média única para um modelo baseado em atividade de viagens observadas, intensidade específica por IMO quando disponível e fallback estatístico registrado quando necessário. Essa mudança não elimina o valor dos modelos por classe ou tipo, mas os reposiciona como fallback e não mais como fonte primária universal.
...
No par Santos--Manaus, as \num{89} observações viagem--OD de \num{22} corredores possuem trabalho positivo. A mediana ponderada de todas elas é \num{9.322050} g/(t$\cdot$nm): \num{40} usam IMO exato e \num{49} usam o fallback de tipo, que concentra \num{59.54}\% do trabalho de transporte. Essas 49 observações idênticas atravessam o percentil ponderado de 50\% e, portanto, controlam o escalar final. O escopo computacional cobre todas as viagens elegíveis, mas isso não equivale a 89 intensidades de combustível medidas de forma independente. O valor permanece \num{17.60}\% acima dos \num{7.926784} g/(t$\cdot$nm) do método anterior Santos--Suape--Manaus. Essa proximidade é reportada como comparação, não usada como critério de ajuste. A média aritmética ponderada das mesmas observações seria \num{25.243618} g/(t$\cdot$nm), inflada por intensidades exatas elevadas, razão pela qual aparece apenas como diagnóstico.
