...
\begin{abstract}
Comparar rodovia e cabotagem exige acompanhar a mesma carga desde o ponto de partida até o destino final. Não basta colocar o consumo de um caminhão ao lado do consumo de um navio. A alternativa com cabotagem também possui um trajeto rodoviário até o primeiro porto e outro trajeto rodoviário depois do último porto. Este artigo apresenta o CabotageLens, um sistema que monta as duas rotas, calcula cada trecho e registra a fonte de cada dado.

O cálculo marítimo começa com as viagens registradas pela Agência Nacional de Transportes Aquaviários (ANTAQ), que informa os portos visitados, as datas, as cargas movimentadas e o número IMO do navio. IMO é a sigla em inglês de \emph{International Maritime Organization}, a Organização Marítima Internacional. O número IMO é um identificador permanente que permite reconhecer a mesma embarcação em bases diferentes. Para calcular uma ligação, o sistema abre cada viagem separadamente e verifica a ordem em que o navio visitou os portos. A viagem \texttt{voyage\_9612791\_00011}, por exemplo, passou por Santos, Suape, Pecém e Manaus, nessa ordem. No cálculo de Santos para Manaus, são usados os três trechos consecutivos desse recorte: Santos--Suape, Suape--Pecém e Pecém--Manaus. Uma viagem em que o navio partiu de Manaus e chegou depois a Santos não representa o transporte no sentido Santos--Manaus e, por isso, fica fora desse cálculo. O método aceita tanto um recorte direto, sem outra escala entre os dois portos escolhidos, quanto um recorte com uma ou mais escalas intermediárias. Um recorte direto pode pertencer a uma viagem completa que possua outras escalas antes ou depois dele. A sequência completa de portos do recorte recebe o nome de corredor.

O sistema também acompanha quanto o navio carrega depois de cada escala. Ele multiplica essa carga pela distância do trecho seguinte. A soma desses produtos é o trabalho de transporte da viagem. Para atribuir uma intensidade de consumo ao navio, o sistema procura seu número IMO na base de Monitoramento, Reporte e Verificação da União Europeia (EU MRV, do inglês \emph{European Union Monitoring, Reporting and Verification}). Essa base publica indicadores anuais de consumo, atividade e emissões de cada embarcação. Quando o IMO não é encontrado, o sistema estima o valor com embarcações semelhantes. Primeiro, procura o grupo mais específico disponível, chamado de classe. Se esse grupo não fornecer um valor, usa uma categoria mais ampla, chamada de tipo, como ``navio porta-contêineres''. Essa substituição recebe o nome de fallback e fica registrada.

Cada recorte fornece uma intensidade e um trabalho de transporte entre os dois portos escolhidos. O sistema ordena essas intensidades e localiza o valor no qual se acumula metade do trabalho total. Esse valor é a mediana ponderada e passa a representar o transporte naquela direção. A direção importa: Santos--Manaus e Manaus--Santos são cálculos diferentes. Para definir a distância da nova remessa, o sistema usa um corredor inteiro observado dentro de uma única viagem, com distância conhecida em todos os subtrechos e um valor de consumo disponível. Se existir um recorte direto, usa esse corredor. Caso contrário, escolhe o corredor completo mais curto entre os que possuem escalas. Os recortes das outras viagens continuam participando da estimativa da intensidade.

A base contém 1.324 viagens, 389 navios identificados por IMO e 177 ligações portuárias com direção definida. Entre Santos e Manaus, o sistema encontrou 89 viagens nas quais o navio saiu de Santos e chegou a Manaus em uma escala posterior da mesma viagem. De cada uma, foi aproveitada somente a parte navegada desde a saída de Santos até a chegada a Manaus. Esses 89 recortes seguiram 22 sequências de portos. A intensidade representativa calculada foi 9,322050 gramas de combustível por tonelada e por milha náutica, unidade abreviada como g/(t$\cdot$nm), em que g significa grama, t significa tonelada e nm significa milha náutica. O único recorte direto observado, pertencente à viagem \texttt{voyage\_9612789\_00004}, saiu de Santos com 11.584,165~t a bordo. A matriz marítima atribuiu 3.300,216~nm a esse subtrecho; essa distância é modelada e não corresponde a uma trajetória medida por posições do navio. O trabalho de transporte foi 38.230.246,479~t$\cdot$nm e, com a intensidade de 9,322050~g/(t$\cdot$nm), o consumo reconstruído foi 356.384,277~kg de combustível. A intensidade veio da média aparada em 1\% do tipo-padrão documentado \emph{container ship}, usado porque não havia correspondência individual aplicável pelo IMO nem classe ou tipo específico informado. Esse valor cobre somente a navegação histórica reconstruída. Ele não inclui os acessos rodoviários nem as operações portuárias. A interpretação ambiental se limita às emissões operacionais do tanque à roda, no caminhão, e do tanque à esteira, no navio (\ttw{}, das expressões inglesas \emph{tank-to-wheel} e \emph{tank-to-wake}), isto é, às emissões geradas pelo uso do combustível. Essa fronteira não inclui as etapas anteriores de produção, processamento, transporte e distribuição do combustível. As emissões são expressas em dióxido de carbono equivalente (\cooe{}), medida que reúne diferentes gases de efeito estufa segundo seu impacto climático. O valor monetário é uma estimativa de custo operacional, e não uma cotação de frete.
