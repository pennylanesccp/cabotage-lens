...
\paragraph{4.3.4.1.3 Reconstrução das
...
\paragraph{4.3.4.1.4 Carga a bordo}\label{carga-a-bordo}}

A carga a bordo de cada subtrecho é a massa transportada pelo navio. Para
a escala \(k\), o sistema calcula:

\[
B_k=B_{k-1}+E_k-D_k,
\]

em que \(B_k\) é a carga total a bordo após a escala \(k\);
\(B_{k-1}\) é a carga existente antes dessa escala; \(E_k\) é a massa
embarcada; e \(D_k\) é a massa desembarcada. O mesmo procedimento é
realizado separadamente para a massa, em toneladas, e para a quantidade
de carga, em TEU.

Em algumas viagens, o saldo acumulado de embarques menos desembarques,
quando calculado a partir de zero, fica negativo em uma ou mais escalas.
Como a carga a bordo não pode ser negativa, o sistema usa o menor valor
desse saldo para determinar a carga inicial mínima compatível com os
registros:
...
Assim, \(B_0\) é a menor carga inicial capaz de manter todos os saldos da
viagem iguais ou superiores a zero. Se o saldo acumulado nunca ficar
negativo, \(B_0\) é igual a zero.

\hypertarget{carga-a-bordo-para-a-viagem-voyage-9612791-00011}{%
\subparagraph{\texorpdfstring{4.3.4.1.4.1 Carga a bordo para a viagem
\texttt{voyage\_9612791\_00011}}{4.3.4.1.4.1 Carga a bordo para a viagem voyage\_9612791\_00011}}\label{carga-a-bordo-para-a-viagem-voyage-9612791-00011}}

A viagem \texttt{voyage\_9612791\_00011} permite mostrar esse
procedimento passo a passo. Para cada escala \(k\), calcula-se primeiro o
saldo da escala, \(\Delta_k=E_k-D_k\), e depois o saldo acumulado
provisório, \(S_k=\sum_{i=1}^{k}(E_i-D_i)\), considerando inicialmente
\(B_0=0\). Os valores de embarque e desembarque vêm da Tabela 4; os dois
saldos são calculados a partir deles.
...
Saldo da escala \(\Delta_k=E_k-D_k\)\strut
...
O desenvolvimento completo dos saldos acumulados é:
...
S_2
&=(9{.}881{,}860-0)
+(11{.}862{,}199-8{.}002{,}620)
=13{.}741{,}439\ \mathrm{t},\\
S_3
&=(9{.}881{,}860-0)
+(11{.}862{,}199-8{.}002{,}620)
\\
&\quad +(3{.}231{,}914-7{.}624{,}347)
=9{.}349{,}006\ \mathrm{t},\\
S_4
&=(9{.}881{,}860-0)
+(11{.}862{,}199-8{.}002{,}620)
\\
&\quad +(3{.}231{,}914-7{.}624{,}347)
+(7{.}571{,}660-19{.}897{,}560)
=-2{.}976{,}894\ \mathrm{t}.
\end{aligned}
\]

O menor saldo acumulado provisório é \(-2{.}976{,}894\)~t. Aplicando a
fórmula da carga inicial mínima:
...
O saldo provisório não representa a carga efetiva a bordo, pois foi
calculado supondo artificialmente que o navio chegou à primeira escala
sem carga. Sua função é identificar a carga inicial mínima necessária
para impedir valores negativos. Somando \(B_0\) aos saldos acumulados,
obtém-se a carga a bordo após cada escala:
...
combustível foi necessário para realizar esse transporte. Conforme
fundamentado na Seção 3.1, o sistema usa a \textbf{intensidade de
combustível} para obter uma estimativa média e comparável, ao invés de
simular as condições particulares de uma única viagem. Essa intensidade
é a quantidade de combustível associada ao transporte de uma tonelada
por uma milha náutica. A unidade é grama por tonelada-milha náutica, ou
\(\mathrm{g/(t\cdot nm)}\).

Segundo a EU MRV, esse indicador é construído a partir de um processo
padronizado. Para cada navio, a companhia define um plano de
monitoramento que descreve os métodos e as fontes de dados utilizados.
Esse plano é avaliado por um verificador acreditado e, quando aplicável,
aprovado pela autoridade responsável. Durante o período de reporte, são
registrados por viagem o combustível consumido, a distância percorrida,
o tempo no mar e a carga transportada. Ao final do ano, esses registros
são consolidados em um relatório de emissões do navio, que também passa
por verificação antes de ser submetido pelo sistema THETIS-MRV
...
em que \(m_v\) é a massa de carga da viagem \(v\), \(d_v\) é a distância
percorrida, \(W_v\) é o respectivo trabalho de transporte e
\(F_{\mathrm{anual}}\) é o combustível total consumido pelo navio no
período. Uma intensidade de
...
Para navios porta-contêineres, o EU MRV permite determinar a carga pela
massa real ou pelo número de TEU multiplicado por pesos-padrão. A opção
adotada deve constar no plano de monitoramento e ser aplicada de forma
consistente durante todo o período de reporte. Os métodos de medição do
combustível, suas fontes de dados e as incertezas associadas também devem
ser documentados; o guia recomenda verificações de plausibilidade e,
quando possível, o uso de uma segunda fonte para conferir os dados
principais \parencite[p. 42, 48--49 e 109--110]{ecmrvguide2025}.

Esses procedimentos conferem rastreabilidade, consistência e verificação
por entidade acreditada ao indicador, tornando-o adequado como
referência histórica para uma avaliação comparativa. Isso não significa
que a intensidade reproduza exatamente uma viagem brasileira ou que
elimine toda incerteza de medição. Ela representa o desempenho anual
verificado do navio dentro das regras do EU MRV. O consumo atribuído a
cada viagem reconstruída pelo CabotageLens é calculado posteriormente,
ao aplicar essa intensidade ao respectivo trabalho de transporte,
conforme a Seção 4.3.4.3.
...
A Figura 5 mostra parte de uma dessas planilhas anuais. Cada linha
...
{\raggedright
\emph{Figura 5 --- Captura de tela de parte do arquivo
...
\subparagraph{4.3.4.2.2 Valores atípicos, P95 e estatística
...
\texttt{Ship type} da publicação do EU MRV. O sistema lê esse campo para
cada número IMO, desconsidera apenas diferenças entre letras maiúsculas
e minúsculas e espaços extras e reúne os rótulos iguais. Assim, todos os
navios identificados como \emph{container ship} formam um grupo. Cada
IMO participa uma única vez, com seu indicador positivo mais recente.
Portanto, o tipo não é inferido pelo nome do navio, pela rota, pelo
armador ou pelas dimensões da embarcação \parencite{emsa2026}.

Depois de formar o grupo, o sistema calcula o percentil 95 (P95), ponto
abaixo do qual estão 95\% dos valores ordenados. Em outras palavras,
apenas os 5\% maiores ficam acima do P95. A regra só é aplicada quando há
pelo menos 20 navios no grupo, para que essa comparação tenha uma base
mínima.
...
marítima representativa é descrito separadamente na Seção~4.3.4.4.

\hypertarget{estimativa-quando-nuxe3o-huxe1-valor-individual}{%
\subparagraph{4.3.4.2.3 Estimativa quando não há valor
...
procura um grupo de embarcações semelhantes.

Neste trabalho, \textbf{classe do navio} é uma faixa de porte definida
pelo modelo dentro do tipo \emph{container ship}. O porte é aproximado
em toneladas com campos do EU MRV ligados ao trabalho de transporte: o
sistema prioriza informações baseadas em \emph{deadweight} (DWT) e,
quando elas não estão disponíveis, usa informações baseadas na massa
transportada. Em seguida, aplica três faixas:
\texttt{container\_small}, abaixo de 20.000 t;
\texttt{container\_feeder}, de 20.000 t a menos de 40.000 t; e
\texttt{container\_large}, a partir de 40.000 t. Esses limites são
regras de modelagem do CabotageLens; não são classes publicadas pelo EU
MRV nem classificações técnicas atribuídas por uma sociedade
classificadora.

A classe é mais específica que o tipo porque separa os
porta-contêineres por faixa de porte. O sistema só usa a referência de
classe quando essa identificação está disponível nos dados do navio ou
da viagem. Ele não deduz uma classe ausente pelo nome ou pela rota. Se a
classe não estiver disponível, procura o tipo, categoria mais ampla,
como \emph{container ship}. O resultado é identificado como estimativa
da classe ou do tipo; ele não é apresentado como se fosse uma medição
individual do navio ausente.
...
antes do cálculo das emissões. Foram selecionados cinco recortes
Santos--Manaus que possuem intensidade individual obtida diretamente
pelo IMO no EU MRV e cuja distância não ultrapassa o P95 de
\(6.975{,}000\ \mathrm{km}\). Portanto, nenhum deles exige substituição
da intensidade por uma estatística de classe ou tipo, nem retirada da
média de distância.

A base atual contém somente quatro IMOs distintos que atendem
simultaneamente a esses critérios. Por isso, o exemplo usa cinco
recortes de quatro navios: o IMO 9612791 aparece em duas viagens, mas
percorre sequências de escalas diferentes. Essa repetição é mantida
porque a unidade observada pelo método é o recorte de cada viagem, e não
apenas o navio.

\begin{center}
\resizebox{\linewidth}{!}{%
\begin{tikzpicture}
  \node[anchor=west,font=\scriptsize\bfseries] at (-1.15cm,1.50cm)
    {V1 · voyage\_9612791\_00011 · IMO 9612791 · \(I=7{,}43\ \mathrm{g/(t\cdot nm)}\)};
  \node[clnode] (v1a) at (0.00cm,0.00cm) {Santos};
  \node[clnode] (v1b) at (7.50cm,0.00cm) {Suape};
  \node[clnode] (v1c) at (15.00cm,0.00cm) {Pecém};
  \node[clnode] (v1d) at (22.50cm,0.00cm) {Manaus};
  \node[align=center,text width=5.80cm,font=\tiny] at (3.75cm,0.72cm)
    {\(m=12.858{,}754\ \mathrm{t}\)\\\(d=2.332{,}000\ \mathrm{km}\mid1.259{,}179\ \mathrm{nm}\)};
  \node[align=center,text width=5.80cm,font=\tiny] at (11.25cm,0.72cm)
    {\(m=16.718{,}333\ \mathrm{t}\)\\\(d=940{,}458\ \mathrm{km}\mid507{,}806\ \mathrm{nm}\)};
  \node[align=center,text width=5.80cm,font=\tiny] at (18.75cm,0.72cm)
    {\(m=12.325{,}900\ \mathrm{t}\)\\\(d=2.195{,}720\ \mathrm{km}\mid1.185{,}594\ \mathrm{nm}\)};
  \draw[clarrow] (v1a) -- (v1b);
  \draw[clarrow] (v1b) -- (v1c);
  \draw[clarrow] (v1c) -- (v1d);

  \node[anchor=west,font=\scriptsize\bfseries] at (-1.15cm,-1.50cm)
    {V2 · voyage\_9612791\_00003 · IMO 9612791 · \(I=7{,}43\ \mathrm{g/(t\cdot nm)}\)};
  \node[clnode] (v2a) at (0.00cm,-3.00cm) {Santos};
  \node[clnode] (v2b) at (4.50cm,-3.00cm) {Itajaí};
  \node[clnode] (v2c) at (9.00cm,-3.00cm) {Paranaguá};
  \node[clnode] (v2d) at (13.50cm,-3.00cm) {Suape};
  \node[clnode] (v2e) at (18.00cm,-3.00cm) {Pecém};
  \node[clnode] (v2f) at (22.50cm,-3.00cm) {Manaus};
  \node[align=center,text width=3.70cm,font=\tiny] at (2.25cm,-2.28cm)
    {\(m=4.917{,}810\ \mathrm{t}\)\\\(d=419{,}000\ \mathrm{km}\mid226{,}242\ \mathrm{nm}\)};
  \node[align=center,text width=3.70cm,font=\tiny] at (6.75cm,-2.28cm)
    {\(m=13.522{,}710\ \mathrm{t}\)\\\(d=193{,}000\ \mathrm{km}\mid104{,}212\ \mathrm{nm}\)};
  \node[align=center,text width=3.70cm,font=\tiny] at (11.25cm,-2.28cm)
    {\(m=10.386{,}517\ \mathrm{t}\)\\\(d=2.573{,}000\ \mathrm{km}\mid1.389{,}309\ \mathrm{nm}\)};
  \node[align=center,text width=3.70cm,font=\tiny] at (15.75cm,-2.28cm)
    {\(m=24.196{,}861\ \mathrm{t}\)\\\(d=940{,}458\ \mathrm{km}\mid507{,}806\ \mathrm{nm}\)};
  \node[align=center,text width=3.70cm,font=\tiny] at (20.25cm,-2.28cm)
    {\(m=22.825{,}450\ \mathrm{t}\)\\\(d=2.195{,}720\ \mathrm{km}\mid1.185{,}594\ \mathrm{nm}\)};
  ...
  \node[anchor=west,font=\scriptsize\bfseries] at (-1.15cm,-4.50cm)
    {V3 · voyage\_9539494\_00001 · IMO 9539494 · \(I=9{,}50\ \mathrm{g/(t\cdot nm)}\)};
  \node[clnode] (v3a) at (0.00cm,-6.00cm) {Santos};
  \node[clnode] (v3b) at (4.50cm,-6.00cm) {Itapoá};
  \node[clnode] (v3c) at (9.00cm,-6.00cm) {Rio de Janeiro};
  \node[clnode] (v3d) at (13.50cm,-6.00cm) {Suape};
  \node[clnode] (v3e) at (18.00cm,-6.00cm) {Pecém};
  \node[clnode] (v3f) at (22.50cm,-6.00cm) {Manaus};
  \node[align=center,text width=3.70cm,font=\tiny] at (2.25cm,-5.28cm)
    {\(m=16.056{,}390\ \mathrm{t}\)\\\(d=420{,}711\ \mathrm{km}\mid227{,}166\ \mathrm{nm}\)};
  \node[align=center,text width=3.70cm,font=\tiny] at (6.75cm,-5.28cm)
    {\(m=21.950{,}396\ \mathrm{t}\)\\\(d=750{,}344\ \mathrm{km}\mid405{,}153\ \mathrm{nm}\)};
  \node[align=center,text width=3.70cm,font=\tiny] at (11.25cm,-5.28cm)
    {\(m=24.195{,}104\ \mathrm{t}\)\\\(d=1.965{,}000\ \mathrm{km}\mid1.061{,}015\ \mathrm{nm}\)};
  \node[align=center,text width=3.70cm,font=\tiny] at (15.75cm,-5.28cm)
    {\(m=22.048{,}947\ \mathrm{t}\)\\\(d=940{,}458\ \mathrm{km}\mid507{,}806\ \mathrm{nm}\)};
  \node[align=center,text width=3.70cm,font=\tiny] at (20.25cm,-5.28cm)
    {\(m=22.144{,}555\ \mathrm{t}\)\\\(d=2.195{,}720\ \mathrm{km}\mid1.185{,}594\ \mathrm{nm}\)};
  ...
  \node[anchor=west,font=\scriptsize\bfseries] at (-1.15cm,-7.50cm)
    {V4 · voyage\_9649835\_00009 · IMO 9649835 · \(I=5{,}41\ \mathrm{g/(t\cdot nm)}\)};
  \node[clnode] (v4a) at (0.00cm,-9.00cm) {Santos};
  \node[clnode] (v4b) at (5.63cm,-9.00cm) {Itapoá};
  \node[clnode] (v4c) at (11.25cm,-9.00cm) {Suape};
  \node[clnode] (v4d) at (16.88cm,-9.00cm) {Pecém};
  \node[clnode] (v4e) at (22.50cm,-9.00cm) {Manaus};
  \node[align=center,text width=4.70cm,font=\tiny] at (2.81cm,-8.28cm)
    {\(m=27.128{,}064\ \mathrm{t}\)\\\(d=420{,}711\ \mathrm{km}\mid227{,}166\ \mathrm{nm}\)};
  \node[align=center,text width=4.70cm,font=\tiny] at (8.44cm,-8.28cm)
    {\(m=26.218{,}790\ \mathrm{t}\)\\\(d=2.596{,}249\ \mathrm{km}\mid1.401{,}862\ \mathrm{nm}\)};
  \node[align=center,text width=4.70cm,font=\tiny] at (14.06cm,-8.28cm)
    {\(m=27.448{,}321\ \mathrm{t}\)\\\(d=940{,}458\ \mathrm{km}\mid507{,}806\ \mathrm{nm}\)};
  \node[align=center,text width=4.70cm,font=\tiny] at (19.69cm,-8.28cm)
    {\(m=26.331{,}352\ \mathrm{t}\)\\\(d=2.195{,}720\ \mathrm{km}\mid1.185{,}594\ \mathrm{nm}\)};
  \draw[clarrow] (v4a) -- (v4b);
  \draw[clarrow] (v4b) -- (v4c);
  \draw[clarrow] (v4c) -- (v4d);
  \draw[clarrow] (v4d) -- (v4e);

  \node[anchor=west,font=\scriptsize\bfseries] at (-1.15cm,-10.50cm)
    {V5 · voyage\_9343974\_00002 · IMO 9343974 · \(I=6{,}80\ \mathrm{g/(t\cdot nm)}\)};
  \node[clnode] (v5a) at (0.00cm,-12.00cm) {Santos};
  \node[clnode] (v5b) at (5.63cm,-12.00cm) {Itapoá};
  \node[clnode] (v5c) at (11.25cm,-12.00cm) {Paranaguá};
  \node[clnode] (v5d) at (16.88cm,-12.00cm) {Suape};
  \node[clnode] (v5e) at (22.50cm,-12.00cm) {Manaus};
  \node[align=center,text width=4.70cm,font=\tiny] at (2.81cm,-11.28cm)
    {\(m=13.420{,}582\ \mathrm{t}\)\\\(d=420{,}711\ \mathrm{km}\mid227{,}166\ \mathrm{nm}\)};
  \node[align=center,text width=4.70cm,font=\tiny] at (8.44cm,-11.28cm)
    {\(m=186{,}001\ \mathrm{t}\)\\\(d=75{,}884\ \mathrm{km}^{*}\mid40{,}974\ \mathrm{nm}^{*}\)};
  \node[align=center,text width=4.70cm,font=\tiny] at (14.06cm,-11.28cm)
    {\(m=184{,}551\ \mathrm{t}\)\\\(d=2.573{,}000\ \mathrm{km}\mid1.389{,}309\ \mathrm{nm}\)};
  \node[align=center,text width=4.70cm,font=\tiny] at (19.69cm,-11.28cm)
    {\(m=955{,}858\ \mathrm{t}\)\\\(d=3.790{,}000\ \mathrm{km}\mid2.046{,}436\ \mathrm{nm}\)};
...
\emph{Figura 6 --- Cinco recortes observados entre Santos e Manaus. Cada
...
sistema. O asterisco identifica o subtrecho Itapoá--Paranaguá, cuja
distância foi estimada por \texttt{haversine\_fallback}; as demais
distâncias vieram da matriz marítima.}
...
\(W_v=\sum_s m_{v,s}d_{v,s}\) aos valores da Figura 6:
...
\begin{aligned}
I_5
&=\frac{\sum_{v=1}^{5}I_vW_v}{\sum_{v=1}^{5}W_v}\\
&=\frac{1.941.386{,}140\times1000}{264.602.033{,}408}\\
...
\begin{aligned}
\bar D_5
&=\frac{\sum_{v=1}^{5}D_v^{\mathrm{km}}\bar m_v}
{\sum_{v=1}^{5}\bar m_v}\\
&=\frac{
\begin{aligned}
&5.468{,}178(13.308{,}588)+
6.321{,}178(16.495{,}245)+
6.272{,}232(22.341{,}035)\\
&\quad+
6.153{,}138(26.509{,}051)+
6.859{,}595(1.422{,}511)
\end{aligned}}
{80.076{,}431}\\
&=6.119{,}690\ \mathrm{km}
...
Para cada etapa, a emissão é o combustível consumido multiplicado pelo
fator correspondente. Nos acessos e nas operações portuárias, usa-se o
volume de Diesel S10; na navegação, usa-se a massa de VLSFO:
...
O custo de cada etapa é o respectivo consumo multiplicado pelo preço do
combustível. No \emph{first mile}, São Paulo é tanto a UF de origem
quanto a UF do porto de Santos, de modo que o preço permanece em
R\$~6,960/L. No \emph{last mile}, a média entre Amazonas e Acre é
\((7{,}250+9{,}270)/2=8{,}260\)~R\$/L. Nas operações portuárias, os
4,820~L correspondem a 2,410~L em cada terminal, e cada parcela recebe o
preço da sua UF. Na navegação, o cálculo mantém a conversão não
arredondada de R\$~3,81231/kg:
...
Figura 7 \parencite{googlemaps2026}. Para o mesmo endereço, o motor de geocodificação retornou as
...
\emph{Figura 7 --- Consulta independente do endereço Avenida Professor
Luciano Gualberto, São Paulo, SP, no Google Maps. Fonte: captura de tela
do Google Maps realizada em 18 de julho de 2026.}

A Figura 8 mostra a consulta, no Google Maps, das coordenadas devolvidas
...
\emph{Figura 8 --- Consulta no Google Maps das coordenadas retornadas
...
A Figura 9 apresenta uma consulta independente no Google Maps para a
...
\emph{Figura 9 --- Consulta no Google Maps para São Paulo--Rio Branco:
...
informados no cenário. A Figura 10 mostra os campos da barra lateral
...
\emph{Figura 10 --- Campos de definição do cenário na página Router.
Fonte: elaboração própria.}

Após a execução, a página apresenta as alternativas calculadas em um
mapa, como ilustra a Figura 11. As linhas traçadas servem apenas para
...
\emph{Figura 11 --- Representação visual das alternativas calculadas na
...
A Figura 12 mostra os campos usados para definir o cenário. No exemplo, a
...
\emph{Figura 12 --- Campos de definição do cenário na página Mapa de
...
obtidos para os destinos próximos. A Figura 13 ilustra essa representação
...
\emph{Figura 13 --- Representação espacial dos resultados calculados na
...
\emph{Figura 14 --- Resultado da calculadora da Aliança para São
...
\emph{Figura 15 --- Resultado da calculadora de emissões da Log-In.
...
\emph{Figura 16 --- Resultado da calculadora de piso mínimo de frete da
