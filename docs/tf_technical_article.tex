...
% Regras de paginação: priorizam blocos completos, mesmo quando isso deixa
% mais espaço em branco ao final da página anterior.
\raggedbottom
\clubpenalty=10000
\widowpenalty=10000
\displaywidowpenalty=10000
\newcommand{\keeptable}{\Needspace{0.64\textheight}}
\newcommand{\keepcalculation}{\Needspace{12\baselineskip}}
\BeforeBeginEnvironment{center}{\Needspace{0.72\textheight}}
\BeforeBeginEnvironment{longtable}{\Needspace{16\baselineskip}}
\BeforeBeginEnvironment{itemize}{\Needspace{10\baselineskip}\begin{samepage}}
\AfterEndEnvironment{itemize}{\end{samepage}}
\BeforeBeginEnvironment{enumerate}{\Needspace{10\baselineskip}\begin{samepage}}
...
{\LARGE\bfseries\color{clblue}\reporttitle\par}

\vfill

{\large Trabalho de Formatura\par}
\vspace{0.5cm}
{\large\textbf{Orientador:} \reportadvisor\par}

\vfill

{\large\reportlocation\par}
{\large\reportyear\par}
\end{titlepage}

\section*{Resumo}

Este trabalho apresenta o CabotageLens, um sistema computacional auditável
para comparar, porta a porta, o transporte rodoviário direto e a alternativa
multimodal formada por acessos rodoviários, operações portuárias e cabotagem
no Brasil. Para a mesma carga, origem e destino, o sistema estima as
distâncias, o consumo de Diesel S10 e VLSFO, as emissões operacionais
\emph{tank-to-wheel} (TTW) e o custo modelado do combustível. A perna
marítima é construída com viagens observadas pela Agência Nacional de
Transportes Aquaviários (ANTAQ): as escalas são ordenadas, a carga a bordo é
calculada em cada subtrecho e a intensidade de combustível é obtida no
sistema europeu de Monitoramento, Reporte e Verificação (EU MRV),
preferencialmente pelo mesmo número IMO ou, quando necessário, por uma
estimativa robusta do tipo do navio. No exemplo de uma remessa de 14~t entre
São Paulo e Rio Branco, a alternativa multimodal percorreu 117,24\% mais
quilômetros, mas apresentou 32,30\% menos emissões operacionais de
CO\textsubscript{2}e e custo modelado do combustível 46,69\% menor que a
alternativa rodoviária direta. Esses resultados se restringem ao cenário e
às premissas declaradas: não representam frete comercial, disponibilidade
do serviço, emissões \emph{well-to-wheel} ou avaliação de ciclo de vida.

\noindent\textbf{Palavras-chave:} cabotagem; transporte multimodal; emissões
operacionais; custo de combustível; rastreabilidade.
...
\textit{Fallback} & Procedimento alternativo acionado quando a fonte ou o método principal não fornece um resultado utilizável; sua aplicação permanece identificada na saída. \\
...
\textit{Pipeline} & Sequência organizada de etapas que recebe os dados do cenário, consulta as fontes necessárias, executa os cálculos e reúne os resultados. \\
RTG & \textit{Rubber-tyred gantry crane}, ou guindaste de pórtico sobre pneus; equipamento usado na movimentação de contêineres no terminal. \\
...
Massa embarcada na escala \(k\) & \(M_k^{\mathrm{emb}}\) & t\tabularnewline
Massa desembarcada na escala \(k\) & \(M_k^{\mathrm{des}}\) &
t\tabularnewline
...
\begin{tikzpicture}
  ...
  \draw[clarrow] (mS) --
    node[midway, above=2mm, fill=white, inner xsep=3pt, inner ysep=1pt,
      font=\scriptsize, text=clblue!80!black] {1.259,179 nm} (mU);
  \draw[clarrow] (mU) --
    node[midway, above=2mm, fill=white, inner xsep=3pt, inner ysep=1pt,
      font=\scriptsize, text=clblue!80!black] {507,806 nm} (mP);
  \draw[clarrow] (mP) --
    node[midway, above=2mm, fill=white, inner xsep=3pt, inner ysep=1pt,
      font=\scriptsize, text=clblue!80!black] {1.185,594 nm} (mM);
\end{tikzpicture}%
}
\end{center}
...
em que \(\Delta_k\) é o saldo da escala, \(M_k^{\mathrm{emb}}\) é a massa
embarcada e \(M_k^{\mathrm{des}}\) é a massa desembarcada. A carga a
bordo após a escala é:
...
procedimento passo a passo. A Tabela 6 parte de \(S_0=0\) e
...
\keeptable
\textbf{Tabela 6 --- Detalhamento do saldo de carga por escala na viagem
...
Embarcado \(M_k^{\mathrm{emb}}\)\strut
\end{minipage} & \begin{minipage}[b]{0.13\columnwidth}\raggedleft
{\small Desembarcado} \(M_k^{\mathrm{des}}\)\strut
...
valores de \(\Delta_k\) apresentados na Tabela 6:
...
\begin{tikzpicture}
  ...
  \draw[clarrow] (cS) --
    node[midway, above=2mm, fill=white, inner xsep=3pt, inner ysep=1pt,
      font=\scriptsize, text=clblue!80!black] {12.858,754~t} (cU);
  \draw[clarrow] (cU) --
    node[midway, above=2mm, fill=white, inner xsep=3pt, inner ysep=1pt,
      font=\scriptsize, text=clblue!80!black] {16.718,333~t} (cP);
  \draw[clarrow] (cP) --
    node[midway, above=2mm, fill=white, inner xsep=3pt, inner ysep=1pt,
      font=\scriptsize, text=clblue!80!black] {12.325,900~t} (cM);
\end{tikzpicture}%
}
\end{center}

\emph{Figura 5 --- Sequência de escalas e carga a bordo nos subtrechos
da parte de ida da viagem
\texttt{voyage\_\allowbreak 9612791\_\allowbreak 00011}. Fonte:
...
for considerada atípica, recorre a uma estimativa robusta do tipo do
navio. A origem do valor selecionado permanece registrada.
...
\(7{,}43\ \mathrm{g/(t\cdot nm)}\) \parencite{emsa2026}. A Tabela 7 mostra os
campos usados nessa correspondência.

\keeptable
\textbf{Tabela 7 --- Dados do EU MRV usados para a viagem
\texttt{voyage\_9612791\_00011}.}
...
houver correspondência individual utilizável, o sistema recorre à
estimativa coletiva pelo tipo. As duas etapas são apresentadas a seguir.
...
identificados como \emph{container ship} pertencem ao mesmo grupo. Quando
há correspondência individual, o tipo vem desse campo e não é inferido
pelo nome do navio, pela rota, pelo armador ou pelas dimensões da
embarcação \parencite{emsa2026}.
...
No grupo de 268 navios classificados como \emph{container ship}, o P95
é \(13{,}409\ \mathrm{g/(t\cdot nm)}\). O navio de IMO 9603221
...
e passa à estimativa coletiva pelo tipo, descrita na subseção seguinte.
...
\hypertarget{estimativa-robusta-pelo-tipo-quando-nuxe3o-huxe1-valor-individual-utilizuxe1vel}{%
\subparagraph{4.3.4.2.3 Estimativa robusta pelo tipo quando não há valor
individual utilizável}\label{estimativa-robusta-pelo-tipo-quando-nuxe3o-huxe1-valor-individual-utilizuxe1vel}}
...
sistema continua usando a base do EU MRV, mas substitui a referência
individual por uma estimativa coletiva do tipo do navio.

O tipo corresponde à categoria registrada no campo \texttt{Ship type},
como \emph{container ship}. Quando o IMO possui correspondência
individual, o sistema usa o tipo informado no EU MRV. Quando não há
correspondência e, portanto, o tipo não pode ser obtido dessa forma, a
matriz adota \emph{container ship} como categoria padrão, de acordo com
o escopo de cabotagem conteinerizada deste estudo.

Para formar a estimativa, o sistema ordena os indicadores positivos mais
recentes dos navios do mesmo tipo. Quando a amostra permite, retira 1\%
das observações em cada extremidade e calcula a média dos valores
restantes. Se a amostra for pequena demais para retirar ao menos um valor
de cada extremidade, usa a mediana do grupo. Essa estatística reduz a
influência de poucos valores extremos sem escolher deliberadamente uma
intensidade menor.

Assim, tanto para um IMO ausente quanto para um valor individual acima
do P95, a saída registra o tipo adotado, a estatística, o tamanho da
amostra e a quantidade de valores retirados. O resultado é apresentado
como estimativa coletiva do tipo, e não como medição individual do
navio.

\hypertarget{exemplo-de-estimativa-pelo-tipo-de-navio}{%
\subparagraph{4.3.4.2.4 Exemplo de estimativa pelo tipo de
...
\texttt{voyage\_9612777\_00005}, realizada pelo navio de IMO 9612777,
contém um recorte entre Santos e Manaus com escalas em Suape e Pecém.
Esse IMO aparece nos registros da ANTAQ, mas não possui correspondência
individual no EU MRV. Por isso, a matriz adota \emph{container ship}
como categoria padrão e usa o grupo formado pelos indicadores desse tipo
publicados no EU MRV.

Para formar esse valor, o sistema reuniu um indicador positivo e mais
recente de cada um dos 268 navios classificados como \emph{container
ship} no EU MRV. Após ordenar os valores, retirou os dois menores e os
dois maiores, conforme a regra de 1\% em cada extremidade. Restaram 264
...
percorre os 264 valores restantes.

Portanto, todos os subtrechos desse recorte recebem a intensidade de
\(6{,}624583\ \mathrm{g/(t\cdot nm)}\). A saída identifica esse número
como uma estimativa baseada no tipo \emph{container ship}, e não como
uma medição do navio de IMO 9612777.
...
\includegraphics[
  ...
  keepaspectratio
]{images/fig07_cinco_recortes_santos_manaus.png}
\end{center}
...
(\(d\)), com valores arredondados ao inteiro mais próximo apenas para
facilitar a leitura da figura. Fonte: elaboração própria com dados de
Carga e Atracação da ANTAQ, intensidades do EU MRV e distâncias da
matriz marítima do sistema.}
\clearpage
...
10. Ele não corresponde ao combustível total dos 89 recortes históricos:
esses recortes servem apenas para preparar a intensidade e a distância
...
\keeptable
\textbf{Tabela 8 --- Fatores de emissão específicos da alternativa
multimodal.}
...
R\$~3.812,31/mt, ou R\$~3,812/kg. A Tabela 9 resume as fontes, os
valores de origem e os preços usados no exemplo.

\Needspace{10\baselineskip}
\keeptable
\textbf{Tabela 9 --- Preços de combustível usados no exemplo São
Paulo--Rio Branco.}
...
10 reúne os resultados das etapas que compõem a alternativa multimodal.
Os cálculos e as fontes de cada etapa estão descritos nas Seções 4.3.1 a
4.3.6.

\keeptable
\textbf{Tabela 10 --- Resultado da alternativa multimodal no exemplo São
Paulo--Rio Branco, para uma remessa de 14~t.}
...
\keeptable
\textbf{Tabela 11 --- Comparação dos resultados totais no exemplo São
Paulo--Rio Branco, para uma remessa de 14~t.}
...
A Tabela 12 apresenta as tecnologias e os serviços essenciais para
...
\keeptable
\textbf{Tabela 12 --- Tecnologias e serviços utilizados na implementação
do CabotageLens.}
...
A Figura 8 mostra como quatro formas de entrada são processadas para
identificar o mesmo local:
...
\emph{Figura 8 --- Fluxo de geocodificação de diferentes formas de
entrada para um mesmo local. Fonte: elaboração própria.}
...
Figura 9 \parencite{googlemaps2026}. Para o mesmo endereço, o motor de geocodificação retornou as
...
\emph{Figura 9 --- Consulta independente do endereço Avenida Professor
Luciano Gualberto, São Paulo, SP, no Google Maps. Fonte: captura de tela
do Google Maps realizada em 18 de julho de 2026.}

A Figura 10 mostra a consulta, no Google Maps, das coordenadas devolvidas
...
\emph{Figura 10 --- Consulta no Google Maps das coordenadas retornadas
...
3.491,431~km. A Figura 11 resume esse processo.
...
\emph{Figura 11 --- Fluxo de geocodificação e consulta da distância
rodoviária no exemplo São Paulo--Rio Branco. Fonte: elaboração própria.}
...
A Figura 12 apresenta uma consulta independente no Google Maps para a
...
\emph{Figura 12 --- Consulta no Google Maps para São Paulo--Rio Branco:
...
\keeptable
\textbf{Tabela 13 --- Recorte bruto da planilha semanal da ANP para
...
emissões operacionais TTW. A Figura 13 reúne essas etapas.
...
\emph{Figura 13 --- Fluxo de cálculo da alternativa rodoviária direta no
exemplo São Paulo--Rio Branco. Fonte: elaboração própria.}
...
robusta de navios do mesmo tipo. A carga a bordo, a distância e a
intensidade permitem calcular o trabalho de transporte e o consumo
estimado de cada viagem.
...
\texttt{data/sea\_matrix.json}. A Figura 14 resume sua preparação.
...
\emph{Figura 14 --- Fluxo de preparação dos dados observados e compilação
da matriz marítima. Fonte: elaboração própria.}

As Tabelas 14 e 15 mostram parte da matriz marítima preparada com dados
reais. As linhas indicam o porto de origem e, as colunas, o porto de
destino.

A Tabela 14 apresenta a distância representativa de cada sentido. Quando
...
\keeptable
\textbf{Tabela 14 --- Distância marítima representativa após a triagem
...
A Tabela 15 apresenta a intensidade média, ponderada pelo trabalho de
transporte.

\keeptable
\textbf{Tabela 15 --- Intensidade média da ligação marítima, ponderada
...
Tabela 16 compara a distância de Haversine usada na seleção com a
distância rodoviária calculada após a escolha de cada porto.

\keeptable
\textbf{Tabela 16 --- Distâncias de seleção e de acesso rodoviário no
...
Santos--Manaus na \texttt{SeaMatrix}. A Tabela~17 mostra o que a matriz
...
\keeptable
\textbf{Tabela 17 --- Informações devolvidas pela matriz marítima para a
...
reúne os totais das duas alternativas para a mesma remessa. A Tabela 18
apresenta o resultado final do exemplo São Paulo--Rio Branco, com 14~t
de carga.

\keeptable
\textbf{Tabela 18 --- Resultado final do cenário São Paulo--Rio Branco.}
...
utilizadas e os avisos que afetam a leitura do resultado. A Tabela~19
exemplifica esse registro no cenário São Paulo--Rio Branco.

\keeptable
\textbf{Tabela 19 --- Informações de rastreabilidade registradas no
exemplo São Paulo--Rio Branco.}
...
\emph{Esses são os mesmos dados consolidados na Tabela 10, na Seção
4.3.7.}
...
informados no cenário. A Figura 15 mostra os campos da barra lateral
...
\emph{Figura 15 --- Campos de definição do cenário na página Router.
Fonte: elaboração própria.}

Após a execução, a página apresenta as alternativas calculadas em um
mapa, como ilustra a Figura 16. As linhas traçadas servem apenas para
...
\emph{Figura 16 --- Representação visual das alternativas calculadas na
...
A Figura 17 mostra os campos usados para definir o cenário. No exemplo, a
...
\emph{Figura 17 --- Campos de definição do cenário na página Mapa de
...
obtidos para os destinos próximos. A Figura 18 ilustra essa representação
...
\emph{Figura 18 --- Representação espacial dos resultados calculados na
...
\emph{Figura 19 --- Resultado da calculadora da Aliança para São
Paulo--Abaetetuba. Fonte: resultado exportado pela ferramenta, fornecido
pelo autor.}

\keeptable
\textbf{Tabela 20 --- Emissões TTW por etapa no cenário São
...
\emph{Figura 20 --- Resultado da calculadora de emissões da Log-In.
Fonte: captura de tela fornecida pelo autor.}

\keeptable
\textbf{Tabela 21 --- Emissões no cenário de referência São Paulo--Rio
Branco.}
...
No resultado multimodal, a distância da Log-In é 11,064 km maior, uma
diferença de 0,15\%, e a emissão de GEE informada é 0,146 t maior, ou
5,3\% em relação ao CabotageLens. Essa proximidade numérica se limita ao
cenário apresentado e não constitui uma validação independente, pois as
ferramentas utilizam fontes, parâmetros e procedimentos próprios.
...
\emph{Figura 21 --- Resultado da calculadora de piso mínimo de frete da
ANTT para a distância de 3.491 km. Fonte: captura de tela fornecida pelo
autor.}

\keeptable
\textbf{Tabela 22 --- Valores para a ligação São Paulo--Rio Branco.}
...
combustível. Por isso, não se espera que sejam iguais. O custo de
combustível calculado pelo sistema, R\$~12.318,68, representa 57,8\% do piso de
R\$~21.308,12. Essa proporção é apresentada apenas para contextualizar a
magnitude dos valores e não constitui uma referência de validação, pois
o piso mínimo também reúne outras parcelas do frete. Portanto, os
resultados não devem ser interpretados como cotações equivalentes.
